...
function createDbConfigError() {
    const error = new Error('Chat service database is not configured. Set SUPABASE_DB_URL.');
    error.status = 503;
    error.expose = true;
    ...
    try {
        ...
        if (shouldAttemptIpv4Override(originalHostname)) {
            ...
            if (ipv4Lookup?.address) {
                ...
                console.log(`[chat-service] Using IPv4 ${ipv4Lookup.address} for ${originalHostname}`);
            }
        }
        ...
        console.warn('[chat-service] Could not apply IPv4 DB host override:', error.message);
    }
